...
Run the following command (if needed, replace the version, in this case `2.17.0`, with the one you are using):
...
- In case of installing from released YAML file just run the following command (if needed, replace the version, in this case `2.17.0`, with the one you are using):
