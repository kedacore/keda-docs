...
- type: azure-eventhub
  metadata:
    ...
    # Required when cloud = Private
    storageAccountEndpoint: blob.airgap.example
    # Required when cloud = Private.
...
- `storageAccountEndpoint` - Blob Storage endpoint of the cloud environment. (Required when `cloud` is set to `Private`, e.g. `blob.airgap.example`)
