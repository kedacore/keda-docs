...
| `keda.scaled.object.ready` | This metric indicates whether a ScaledObject is ready (value == 1) or not (value == 0), based on its `Ready` status condition. |
| `keda.scaled.job.ready` | This metric indicates whether a ScaledJob is ready (value == 1) or not (value == 0), based on its `Ready` status condition. |
