...
- `unsafeSsl` - Used for skipping certificate check e.g: using self-signed certs. Note that this is currently ignored when `ca` or a client certificate is configured through a `TriggerAuthentication`. (Values: `true`,`false`, Default: `false`, Optional)
...
- `clientSecret` - Client secret for the OAuth2 provider. Only optional if the token endpoint authenticates the client with mutual TLS ([RFC 8705](https://datatracker.ietf.org/doc/html/rfc8705)), in which case the token request carries no client secret at all, leaving the client certificate as the only client authentication method - KEDA logs a warning and emits an event when neither `clientSecret` nor `tls` is configured, because such a setup is effectively unauthenticated.
...
Only the client credentials grant is supported. The access token is acquired once and refreshed when it expires, so it is reused across polling intervals instead of being fetched for every query. Requests to the token endpoint are made with the same HTTP client as the queries themselves, so they share the trigger's `timeout` and its TLS configuration (`ca`, client certificates, `unsafeSsl`).
