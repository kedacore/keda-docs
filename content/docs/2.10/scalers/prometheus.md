...
- type: prometheus
  metadata:
    ...
    cortexOrgID: my-org # Deprecated: Use custom headers instead to set X-Scope-OrgID header for Cortex. (see below)
...
- `cortexOrgID` - **Deprecated in favor of `customHeaders`** and will be removed in a future release. Use `customHeaders: X-Scope-OrgID=##` instead to query multi tenant [Cortex](https://cortexmetrics.io/) or [Mimir](https://grafana.com/oss/mimir/). (Optional)
